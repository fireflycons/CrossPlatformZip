namespace Firefly.CrossPlatformZip
{
    ...
    public class Zipper
    {
        ...
        /// </summary>
        /// <param name="zipFile">The zip file.</param>
        /// <param name="path">The path to a file or folder to zip.</param>
        ...
        /// Value cannot be null or empty - zipFile
        /// </exception>
        /// <exception cref="System.IO.FileNotFoundException">No files found to zip at '{path}</exception>
        public static void Zip(string zipFile, string path, ZipPlatform targetPlatform)
        {
            ...
            using (var archive = CreateZipFile(zipFile))
            {
        ...
        private static ZipArchive CreateZipFile(string zipFile)
        {
            if (Environment.UserInteractive)
            {
                Console.WriteLine($"Creating zip file: {zipFile}");
            }
        ...
        /// Useful for e.g. creating a NodeJS AWS lambda package from a web packed project where the entry should be called index.js
        /// </summary>
        /// <param name="zipFile">The zip file.</param>
        /// <param name="filePath">The file path.</param>
        /// <param name="alternateName">Name of entry to create in zip directory, or if <c>null</c>, use the original file name.</param>
            ...
            string alternateEntryName = null)
        {
            ...
            var zipPath =
                (targetPlatform == ZipPlatform.Unix ? relative.Replace('\\', directorySeparator) : relative.Replace('/', '\\'))
                .TrimEnd('\\', '/');
